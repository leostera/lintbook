...
use lintbook_config::{LintbookConfig, LlmLintConfig};
...
impl LlmLinter {
    pub fn new(config: LintbookConfig, cache_dir: PathBuf) -> Result<Self> {
    ...
    pub async fn run_llm_lints(&self, files: &[PathBuf]) -> Result<Vec<LlmLintResult>> {
        let mut results = Vec::new();

        for (lint_name, lint_config) in self.config.llm.clone() {
        ...
        let filtered_files: Vec<PathBuf> = files
            ...
            .filter(|file| {
                ...
                if let Some(extension) = file.extension().and_then(|e| e.to_str()) {
                    ...
                    language_matches && matches_file_pattern(file, &lint_config.files)
                } else {
                    false
                }
            })
    ...
    async fn run_lint_on_file(
        ...
        self.query_generator
            .validate_query(&lint_config.lang, &query_str)?;
            ...
            for capture in match_.captures {
                ...
                let is_error = self
                    ...
                    .get_or_compute(&validation_prompt, || async {
                        self.provider
                            .validate_match(node_text, &lint_config.error)
                            .await
                    })
                    .await?;
...
fn matches_file_pattern(file: &Path, pattern: &str) -> bool {
    if pattern == "*" || pattern == "**/*" {
        return true;
    }

    if let Some(extension_pattern) = pattern.strip_prefix("**/*.") {
        return file
            .extension()
            .and_then(|extension| extension.to_str())
            .map(|extension| extension == extension_pattern)
            .unwrap_or(false);
    }
    ...
    file.to_string_lossy() == pattern
}

#[cfg(test)]
mod tests {
    use super::*;
    use lintbook_config::{LintbookSection, LlmProviderConfig};
    use std::collections::HashMap;
    use tempfile::tempdir;

    fn create_test_config() -> LintbookConfig {
