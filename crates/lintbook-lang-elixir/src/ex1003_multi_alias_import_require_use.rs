...
impl MultiAliasImportRequireUse {
    ...
    fn parse_module_argument(
        &self,
        args_node: Node,
        source: &str,
        statement_type: &str,
    ) -> Option<AliasStatement> {
    ...
    fn group_by_base_module(
        &self,
        statements: &[AliasStatement],
    ) -> std::collections::HashMap<String, Vec<AliasStatement>> {
        let mut groups = std::collections::HashMap::new();

        for statement in statements {
            groups
                .entry(statement.base_module.clone())
        ...
        groups
            ...
            .map(|(base, statements)| {
                ...
                for stmt in statements {
                    type_groups
                        .entry(stmt.statement_type.clone())
                        .or_insert_with(Vec::new)
                        .push(stmt);
                }
...
"#;
        ...
        assert!(violations[0]
            .message
            .contains("MyApp.{User, Post, Comment}"));
    }
...
"#;
        ...
        assert!(violations[0]
            .message
            .contains("MyApp.{User, Session, Token}"));
    }
}
