...
impl ParameterPatternMatching {
    ...
    fn check_function_patterns(
        ...
        let key_matching: Vec<_> = patterns
            .iter()
            .filter(|p| p.style == PatternStyle::KeyMatching)
            .collect();
        let explicit_naming: Vec<_> = patterns
            .iter()
            .filter(|p| p.style == PatternStyle::ExplicitNaming)
            .collect();
