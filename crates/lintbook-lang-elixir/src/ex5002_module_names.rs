...
impl ModuleNames {
    ...
    fn extract_module_name(&self, node: Node, source: &str) -> Option<String> {
        match node.kind() {
            ...
            "call" => Some(source[node.start_byte()..node.end_byte()].to_string()),
            "arguments" => {
                for i in 0..node.child_count() {
                    if let Some(child) = node.child(i) {
                        if child.is_named() {
                            return self.extract_module_name(child, source);
                        }
                    }
                }
                None
    ...
    fn to_pascal_case(&self, name: &str) -> String {
        ...
        name.split('_')
            ...
            .map(|part| {
                ...
                match chars.next() {
                    Some(first) => {
                        let mut converted = first.to_ascii_uppercase().to_string();
                        converted.push_str(chars.as_str());
                        converted
                    }
                    None => String::new(),
                }
