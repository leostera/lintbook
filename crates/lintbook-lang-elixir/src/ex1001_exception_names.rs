...
impl ExceptionNames {
    ...
    fn extract_module_name(&self, node: Node, source: &str) -> Option<String> {
        ...
        let module_node = self.first_named_descendant(child)?;

        // Extract the text of the alias
        let alias_text = &source[module_node.start_byte()..module_node.end_byte()];
    ...
    fn defines_exception(&self, node: Node, source: &str) -> bool {
        if self.is_named_call(node, source, "defexception") {
            return true;
        }

        for i in 0..node.child_count() {
            if let Some(child) = node.child(i) {
                if self.defines_exception(child, source) {
                    return true;
                }
            }
        }
    ...
    fn is_named_call(&self, node: Node, source: &str, name: &str) -> bool {
        if node.kind() != "call" {
            return false;
        }
    ...
    fn first_named_descendant<'a>(&self, node: Node<'a>) -> Option<Node<'a>> {
        if node.is_named() && node.kind() != "arguments" {
            return Some(node);
        }

        for i in 0..node.child_count() {
            if let Some(child) = node.child(i) {
                if let Some(found) = self.first_named_descendant(child) {
                    return Some(found);
                }
            }
        }
...
mod tests {
    ...
    fn test_consistent_suffix_naming() {
        let code = fs::read_to_string("fixtures/exception_names_consistent.ex")
    ...
    fn test_inconsistent_naming_patterns() {
        let code = fs::read_to_string("fixtures/exception_names_violations.ex")
            .expect("Failed to read fixture file");
    ...
    fn test_no_exception_modules() {
        let code =
            fs::read_to_string("fixtures/no_exceptions.ex").expect("Failed to read fixture file");
