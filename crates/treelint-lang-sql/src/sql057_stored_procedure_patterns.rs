...
impl StoredProcedurePatterns {
    fn check_procedure_patterns(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if lower_line.contains("@") && (lower_line.contains("varchar") || lower_line.contains("int") || lower_line.contains("datetime")) {
    ...
    fn check_function_definition(
        ...
        if lower_line.contains("create function") && !lower_line.contains("returns") {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "Function without RETURNS clause. Specify return type for clarity".to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        }
    ...
    fn check_return_patterns(
        ...
        if lower_line.trim() == "return" {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "RETURN without value. Specify return value or use RETURN 0 for success".to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        }
    ...
    fn check_procedure_calls(
        ...
        if (lower_line.contains("exec ") || lower_line.contains("execute ")) && lower_line.contains("'") {
