...
impl CursorPatterns {
    fn check_cursor_patterns(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if lower_line.contains("open ") {
                cursor_opened = true;
            }
            
            if lower_line.contains("close ") {
                cursor_closed = true;
            }
        ...
        if cursor_declared && !cursor_deallocated {
            ...
            violations.push(LintViolation {
                ...
                message: "Cursor declared but not deallocated. Ensure proper cleanup with DEALLOCATE".to_string(),
    ...
    fn check_cursor_declaration(
        ...
        violations.push(LintViolation {
            ...
            column: start_pos.column + 1,
            message: "Cursor detected. Consider set-based operations for better performance".to_string(),
            lint_name: self.name().to_string(),
            lint_id: self.id().to_string(),
        });
    ...
    fn check_fetch_operations(
        ...
        if lower_line.contains("fetch") && !lower_line.contains("@@fetch_status") {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "FETCH operation without @@FETCH_STATUS check. Ensure proper loop termination".to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        }
        ...
        if lower_line.contains("fetch last") || lower_line.contains("fetch prior") || 
           lower_line.contains("fetch first") || lower_line.contains("fetch absolute") {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
