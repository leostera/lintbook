...
impl OperatorsSpacing {
    ...
    fn check_operator_in_line(
        ...
        while let Some(pos) = line[search_start..].find(operator) {
            ...
            if needs_space_before || needs_space_after {
                ...
                let mut message = format!("Missing space around {} operator", op_name);
                
                if needs_space_before && needs_space_after {
                    message = format!("Missing spaces around {} operator", op_name);
                } else if needs_space_before {
                    message = format!("Missing space before {} operator", op_name);
                } else {
                    message = format!("Missing space after {} operator", op_name);
                }
    ...
    fn should_skip_operator(&self, line: &str, pos: usize, operator: &str) -> bool {
        ...
        if operator == "-" {
            if pos == 0 || line.chars().nth(pos - 1).map_or(false, |c| c.is_whitespace() || c == '(') {
                // Check if it's followed by a digit
                if line.chars().nth(pos + 1).map_or(false, |c| c.is_ascii_digit()) {
                    return true;
                }
            }
        }
        ...
        let context_before = if pos >= 3 { &line[pos.saturating_sub(3)..pos] } else { &line[..pos] };
        let context_after = &line[pos + operator.len()..].chars().take(3).collect::<String>();
