...
impl SchemaValidation {
    ...
    fn check_column_constraints(
        ...
        if lower_line.contains(" bit ") || lower_line.contains(" boolean ") {
            if !lower_line.contains("default") {
                ...
                violations.push(LintViolation {
                    line: start_pos.row + line_idx + 1,
                    column: start_pos.column + 1,
                    message: "Boolean column without default value. Consider adding DEFAULT constraint".to_string(),
                    lint_name: self.name().to_string(),
                    lint_id: self.id().to_string(),
                });
            }
        ...
        let has_created_at = lower_line.contains("created_at") || lower_line.contains("created_date");
        let has_updated_at = lower_line.contains("updated_at") || lower_line.contains("modified_date");
    ...
    fn check_foreign_key_patterns(
        ...
        if lower_line.contains("foreign key") || lower_line.contains("references") {
            ...
            if !lower_line.contains("_id") {
                ...
                violations.push(LintViolation {
                    line: start_pos.row + line_idx + 1,
                    column: start_pos.column + 1,
                    message: "Foreign key column should follow naming convention (e.g., table_name_id)".to_string(),
                    lint_name: self.name().to_string(),
                    lint_id: self.id().to_string(),
                });
            }
        ...
        if lower_line.contains("references") && !lower_line.contains("cascade") && !lower_line.contains("restrict") {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
