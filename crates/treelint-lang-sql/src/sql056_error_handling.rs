...
impl ErrorHandling {
    fn check_error_patterns(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if in_transaction {
                ...
                self.check_transaction_error_handling(&lower_line, line_idx, node, violations, has_try_catch);
            }
        ...
        if in_transaction && !has_try_catch {
            ...
            violations.push(LintViolation {
                ...
                message: "Transaction without error handling. Consider wrapping in TRY-CATCH block".to_string(),
    ...
    fn check_try_catch_patterns(
        ...
        if lower_line.contains("try") && lower_line.matches("try").count() > 1 {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "Nested TRY blocks detected. Consider simplifying error handling logic".to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        }
    ...
    fn check_transaction_error_handling(
        ...
        if lower_line.contains("catch") && !lower_line.contains("rollback") {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "CATCH block in transaction context. Ensure ROLLBACK is called on error".to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        }
    ...
    fn check_exception_info(
        ...
        if lower_line.contains("catch") {
            let error_functions = ["error_message()", "error_number()", "error_severity()", "error_state()"];
        ...
        if lower_line.contains("catch") || lower_line.contains("raiserror") {
            if !lower_line.contains("log") && !lower_line.contains("print") && !lower_line.contains("insert") {
                let start_pos = node.start_position();
                violations.push(LintViolation {
                    line: start_pos.row + line_idx + 1,
                    column: start_pos.column + 1,
