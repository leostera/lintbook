...
fn test_sql001_table_aliasing_style() {
    let rule = treelint_lang_sql::sql001_table_aliasing_style::TableAliasingStyle;
...
fn test_sql002_column_aliasing_style() {
    let rule = treelint_lang_sql::sql002_column_aliasing_style::ColumnAliasingStyle;
...
fn test_sql003_keyword_capitalization() {
    let rule = treelint_lang_sql::sql003_keyword_capitalization::KeywordCapitalization;
...
fn test_sql005_not_equal_consistency() {
    let rule = treelint_lang_sql::sql005_not_equal_operator_consistency::NotEqualOperatorConsistency;
...
fn test_sql007_is_null_vs_equal_null() {
    let rule = treelint_lang_sql::sql007_is_null_vs_equal_null::IsNullVsEqualNull;
...
fn test_sql008_explicit_union_all() {
    let rule = treelint_lang_sql::sql008_explicit_union_all::ExplicitUnionAll;
...
fn test_sql009_explicit_join_types() {
    let rule = treelint_lang_sql::sql009_explicit_join_types::ExplicitJoinTypes;
...
fn test_sql010_redundant_else_null() {
    let rule = treelint_lang_sql::sql010_remove_redundant_else_null::RemoveRedundantElseNull;
...
fn test_sql012_keywords_as_identifiers() {
    let rule = treelint_lang_sql::sql012_keywords_as_identifiers::KeywordsAsIdentifiers;
...
fn test_sql018_trailing_commas() {
    let rule = treelint_lang_sql::sql018_trailing_commas::TrailingCommas;
...
fn test_sql022_quote_style_consistency() {
    let rule = treelint_lang_sql::sql022_quote_style_consistency::QuoteStyleConsistency;
...
fn test_sql024_wildcard_ambiguity() {
    let rule = treelint_lang_sql::sql024_wildcard_column_ambiguity::WildcardColumnAmbiguity;
    
    // Should find violations for SELECT * in joins
    let violations = check_violations(&rule, "SELECT * FROM users JOIN orders ON users.id = orders.user_id");
    assert!(!violations.is_empty());
}

#[test]
fn test_sql026_table_alias_length() {
    let rule = treelint_lang_sql::sql026_table_alias_length::TableAliasLength;
    
    // Should find violations for single-letter aliases in complex queries
    let violations = check_violations(&rule, "SELECT * FROM users u JOIN orders o ON u.id = o.user_id");
    assert!(!violations.is_empty());
    
    // Should not find violations for meaningful aliases
    let violations = check_violations(&rule, "SELECT * FROM users usr JOIN orders ord ON usr.id = ord.user_id");
    assert!(violations.is_empty());
}

#[test]
fn test_sql027_avoid_aliases_in_ctes() {
    let rule = treelint_lang_sql::sql027_avoid_aliases_in_ctes::AvoidAliasesInCtes;
    
    // Should find violations for aliased CTEs
    let violations = check_violations(&rule, "WITH user_data AS (SELECT * FROM users) SELECT * FROM user_data ud");
    assert!(!violations.is_empty());
}

#[test]
fn test_sql028_column_names_in_group_order_by() {
    let rule = treelint_lang_sql::sql028_column_names_in_group_order_by::ColumnNamesInGroupOrderBy;
    
    // Should find violations for positional references
    let violations = check_violations(&rule, "SELECT name, COUNT(*) FROM users GROUP BY 1 ORDER BY 2");
    assert!(!violations.is_empty());
}

#[test]
fn test_sql029_quoted_literals() {
    let rule = treelint_lang_sql::sql029_quoted_literals::QuotedLiterals;
...
fn test_sql030_distinct_values_in_clause() {
    let rule = treelint_lang_sql::sql030_distinct_values_in_clause::DistinctValuesInClause;
