...
impl InvalidAllObject {
    ...
    fn is_all_identifier(&self, node: Node, source: &str) -> bool {
        ...
        node.utf8_text(source.as_bytes()).map_or(false, |text| text == "__all__")
    }
    ...
    fn check_all_value(&self, value_node: Node, source: &str, violations: &mut Vec<LintViolation>) {
        match value_node.kind() {
            ...
            "tuple" => {
                ...
                self.report_violation(value_node, "Use list instead of tuple for __all__", violations);
                self.check_list_contents(value_node, source, violations);
            },
            "list_comprehension" => {
                // List comprehensions are invalid for __all__
                self.report_violation(value_node, "Use simple list of string literals instead of comprehension", violations);
            },
            _ => {
    ...
    fn check_list_contents(&self, list_node: Node, source: &str, violations: &mut Vec<LintViolation>) {
        ...
        match element.kind() {
            ...
            "identifier" => {
                ...
                if let Ok(text) = element.utf8_text(source.as_bytes()) {
                    self.report_violation(element, 
                        &format!("Use string literal \"{}\" instead of variable reference", text), 
                        violations);
                }
            },
            "integer" | "float" => {
            ...
            "formatted_string" => {
                // Invalid: f-string
                self.report_violation(element, "Use plain string literal instead of f-string", violations);
            },
            "binary_operator" => {
                // Invalid: string concatenation or other operations
                self.report_violation(element, "Use plain string literal instead of expression", violations);
            },
            "call" => {
                // Invalid: function call
                self.report_violation(element, "Use string literal instead of function call", violations);
            },
            "attribute" => {
                // Invalid: attribute access
                self.report_violation(element, "Use string literal instead of attribute access", violations);
            },
            "dictionary" => {
                // Invalid: dictionary
                self.report_violation(element, "Use string literal instead of dictionary", violations);
            },
            "list" => {
                // Invalid: nested list
                self.report_violation(element, "Use string literal instead of nested list", violations);
            },
            "tuple" => {
                // Invalid: tuple
            ...
            "list_comprehension" => {
                ...
                self.report_violation(element, "Use simple list of string literals instead of comprehension", violations);
            },
            "," | "[" | "]" | "(" | ")" => {
                // Skip punctuation
            },
            _ => {
                ...
                if let Ok(text) = element.utf8_text(source.as_bytes()) {
                    if !text.trim().is_empty() && text != "," {
                        self.report_violation(element, "Use string literal instead of complex expression", violations);
                    }
                }
            }
        }
...
"#;
        ...
        assert!(violations.iter().any(|v| v.message.contains("function call")));
    }
...
"#;
        ...
        assert!(violations.iter().all(|v| v.message.contains("variable reference")));
    }
...
"#;
        ...
        assert!(violations.iter().any(|v| v.message.contains("comprehension")));
    }
