...
impl DuplicateBases {
    ...
    fn check_class_definition(&self, node: Node, source: &str, violations: &mut Vec<LintViolation>) {
                ...
                if nodes.len() > 1 {
                    ...
                    for node in nodes {
                        ...
                        violations.push(LintViolation {
                            ...
                            message: format!("Duplicate base class '{}' in class definition.", base_name),
    ...
    fn collect_base_classes<'a>(&self, argument_list: Node<'a>, source: &str, base_classes: &mut HashMap<String, Vec<Node<'a>>>) {
        for i in 0..argument_list.child_count() {
            if let Some(child) = argument_list.child(i) {
                if let Some(base_name) = self.extract_base_class_name(child, source) {
                    base_classes.entry(base_name).or_insert_with(Vec::new).push(child);
                }
            }
        }
    }
...
"#;
        ...
        assert!(violations.iter().all(|v| v.message.contains("package.module.Class")));
    }
