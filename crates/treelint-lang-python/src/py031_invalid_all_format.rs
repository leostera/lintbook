...
impl InvalidAllFormat {
    fn visit_node(&self, node: Node, source: &str, violations: &mut Vec<LintViolation>, depth: usize) {
        let is_scope_creating = matches!(node.kind(), 
        ...
        match node.kind() {
            "assignment" => {
                if let Some(target) = self.find_all_assignment_target(node) {
                    if self.is_all_identifier(target, source) {
                        ...
                        if depth > 0 {
                            self.report_violation(node, "__all__ should only be assigned at module level", violations);
                        }
                    }
                }
            },
            "expression_statement" => {
                ...
                if let Some(call_node) = self.find_child_by_kind(node, "call") {
                    if self.is_all_method_call(call_node, source) {
                        self.report_violation(call_node, "__all__ should not be modified dynamically", violations);
                    }
                }
            },
            "augmented_assignment" => {
                ...
                if let Some(target) = node.child(0) {
                    if self.is_all_identifier(target, source) {
                        self.report_violation(node, "__all__ should not use augmented assignment operators", violations);
                    }
                }
            },
            "named_expression" => {
                ...
                if let Some(target) = node.child(0) {
                    if self.is_all_identifier(target, source) {
                        self.report_violation(node, "__all__ should not be used with walrus operator", violations);
                    }
                }
            },
            _ => {}
        ...
        for i in 0..node.child_count() {
            if let Some(child) = node.child(i) {
                let new_depth = if is_scope_creating {
                    depth + 1
                } else {
                    depth
                };
...
"#;
        ...
        assert!(violations.iter().all(|v| v.message.contains("module level")));
    }
