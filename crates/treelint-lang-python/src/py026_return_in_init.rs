...
impl ReturnInInit {
    ...
    fn check_function_definition(&self, node: Node, source: &str, violations: &mut Vec<LintViolation>) {
    ...
    fn find_return_statements(&self, node: Node, source: &str, violations: &mut Vec<LintViolation>) {
        if node.kind() == "return_statement" {
            ...
            if self.has_return_value(node, source) {
                ...
                violations.push(LintViolation {
                    ...
                    message: "Return with value in __init__ method. __init__ should not return a value.".to_string(),
