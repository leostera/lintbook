...
impl DefaultExceptNotLast {
                ...
                ExceptionType::Exception => {
                    ...
                    if found_bare_except {
                        ...
                        violations.push(LintViolation {
                            ...
                            message: "Exception catch after bare except. Bare except should be last.".to_string(),
    ...
    fn get_exception_type(&self, except_clause: Node, source: &str) -> ExceptionType {
        // Look for the exception type in the except clause
        // We need to find the first identifier that represents the exception type
        self.find_exception_identifier(except_clause, source)
    }
...
"#;
        ...
        assert!(violations[0].message.contains("Specific except clause after bare except"));
    }
...
"#;
        ...
        assert_eq!(violations.len(), 1);
        assert_eq!(violations[0].lint_id, "PY024");
        assert!(violations[0].message.contains("Exception catch after bare except"));
    }
...
"#;
        ...
        assert_eq!(violations.len(), 1);
        assert_eq!(violations[0].lint_id, "PY024");
        assert!(violations[0].message.contains("Specific except clause after Exception"));
    }
