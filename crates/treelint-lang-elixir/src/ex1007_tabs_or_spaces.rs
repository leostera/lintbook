            ...
            IndentationStyle::Mixed { mixed_lines } => {
                for &line_num in &mixed_lines {
                    violations.push(LintViolation {
                        line: line_num,
                        column: 1,
                        message: "Mixed indentation: line uses both tabs and spaces for indentation".to_string(),
                        lint_name: self.name().to_string(),
                        lint_id: self.id().to_string(),
                    });
                }
            }
            IndentationStyle::Inconsistent { tab_lines, space_lines } => {
...
enum IndentationStyle {
    Consistent,
    NoIndentation,
    Mixed { mixed_lines: Vec<usize> },
    Inconsistent { tab_lines: Vec<usize>, space_lines: Vec<usize> },
}
...
impl TabsOrSpaces {
    fn analyze_indentation(&self, source: &str) -> IndentationStyle {
        ...
        if !tab_lines.is_empty() && !space_lines.is_empty() {
            return IndentationStyle::Inconsistent { tab_lines, space_lines };
        }
...
mod tests {
    ...
    fn test_detects_mixed_indentation_within_line() {
        ...
        assert!(violations.iter().any(|v| v.message.contains("Mixed indentation")));
    ...
    fn test_detects_inconsistent_indentation_across_lines() {
        ...
        assert!(!violations.is_empty());
        assert!(violations.iter().any(|v| v.message.contains("Inconsistent indentation")));
    }
...
defmodule Example do
  def test do
    ...
    fn test_reports_minority_style_as_violations() {
        ...
        assert!(violations.iter().any(|v| v.line == 6 && v.message.contains("this line uses tabs")));
        assert!(violations.iter().any(|v| v.line == 7 && v.message.contains("this line uses tabs")));
        assert!(violations.iter().any(|v| v.line == 8 && v.message.contains("this line uses tabs")));
    }
}