...
impl SpaceInParentheses {
                ...
                ')' | ']' | '}' if !in_string && !in_charlist => {
                    ...
                    if let Some((bracket_type, open_pos)) = stack.pop() {
                        if bracket_type == expected_type {
                            ...
                            let has_space_before_close = i > 0 && 
    ...
    fn check_consistency(&self, pairs: &[BracketPair], violations: &mut Vec<LintViolation>) {
        ...
        for bracket_type in [BracketType::Parentheses, BracketType::SquareBrackets, BracketType::CurlyBraces] {
            let type_pairs: Vec<_> = pairs.iter().filter(|p| p.bracket_type == bracket_type).collect();
            ...
            let with_spaces = type_pairs.iter().filter(|p| p.has_space_after_open && p.has_space_before_close).count();
            let without_spaces = type_pairs.iter().filter(|p| !p.has_space_after_open && !p.has_space_before_close).count();
                ...
                if current_mixed || (current_has_spaces != prefer_spaces) {
                    ...
                    violations.push(LintViolation {
                        ...
                        message: format!(
                            ...
                            bracket_name,
                            expected_style,
                            if current_mixed { " (currently mixed)" } else { "" }
                        ),
...
"#;
        ...
        assert!(violations.iter().any(|v| v.message.contains("Inconsistent spacing in parentheses")));
    }
...
"#;
        ...
        assert!(!violations.is_empty());
        assert!(violations.iter().any(|v| v.message.contains("square brackets")));
    }
...
"#;
        ...
        assert!(!violations.is_empty());
        assert!(violations.iter().any(|v| v.message.contains("curly braces")));
    }
