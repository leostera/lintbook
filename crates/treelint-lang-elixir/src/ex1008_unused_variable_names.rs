...
impl Rule for UnusedVariableNames {
    ...
    fn check(&self, tree: &Tree, source: &str) -> Vec<LintViolation> {
        ...
        match style_analysis {
            UnusedVariableStyle::Inconsistent { anonymous_vars, meaningful_vars } => {
                ...
                let (minority_vars, minority_style, majority_style) = 
                    if anonymous_vars.len() <= meaningful_vars.len() {
                        (anonymous_vars, "anonymous underscore", "meaningful underscore names")
                    } else {
                        (meaningful_vars, "meaningful underscore names", "anonymous underscore")
                    };
...
impl UnusedVariableNames {
    ...
    fn traverse_for_unused_variables(&self, node: Node, source: &str, unused_vars: &mut Vec<UnusedVariable>) {
    ...
    fn analyze_naming_style(&self, unused_vars: &[UnusedVariable]) -> UnusedVariableStyle {
        ...
        let anonymous_vars: Vec<_> = unused_vars.iter()
            .filter(|var| var.is_anonymous)
            .cloned()
            .collect();
            
        let meaningful_vars: Vec<_> = unused_vars.iter()
            .filter(|var| !var.is_anonymous)
            .cloned()
            .collect();
...
defmodule Example do
  def test(_user, data) do
    ...
    result
        ...
        assert!(violations.iter().any(|v| v.message.contains("Inconsistent unused variable naming")));
    }
...
defmodule Example do
  def test(_user, data) do
    ...
    result
        ...
        assert!(violations.iter().any(|v| v.message.contains("anonymous underscore") && v.message.contains("meaningful underscore names")));
...
defmodule Example do
  def test(_, data) do
    ...
    result
        ...
        assert!(violations.iter().any(|v| v.message.contains("_single_meaningful")));
        assert!(violations.iter().any(|v| v.message.contains("meaningful underscore names") && v.message.contains("anonymous underscore")));
    }
