...
pub struct LintbookConfig {
    pub lintbook: LintbookSection,
    #[serde(default)]
    pub lints: HashMap<String, HashMap<String, bool>>,
    #[serde(default, skip_serializing_if = "HashMap::is_empty")]
    pub llm: HashMap<String, LlmLintConfig>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct LintbookSection {
...
mod tests {
    ...
    fn test_new_config() {
        ...
        let config = LintbookConfig::new(grammars);

        assert_eq!(config.lintbook.languages.len(), 3);
        assert!(config.lintbook.languages.contains(&"python".to_string()));
        assert!(config.lintbook.languages.contains(&"rust".to_string()));
        assert!(config
            .lintbook
            .languages
            .contains(&"javascript".to_string()));
        assert_eq!(config.lintbook.autofix, true);
        assert!(config.lints.is_empty());
    }

    #[test]
    fn test_is_lint_enabled_default() {
        let config = LintbookConfig::new(vec!["python"]);
    ...
    fn test_is_lint_enabled_explicit() {
        let mut config = LintbookConfig::new(vec!["python"]);
    ...
    fn test_write_and_read_config() {
        ...
        let original_config = LintbookConfig::new(vec!["python", "rust"]);
        original_config.write_to_file(&config_path).unwrap();

        let loaded_config = LintbookConfig::read_from_file(&config_path).unwrap();

        assert_eq!(loaded_config.lintbook.languages.len(), 2);
        ...
        assert_eq!(loaded_config.lintbook.autofix, true);
    }
