...
impl UnusedImport {
    ...
    fn process_import_statement(&self, node: Node, source: &str, imports: &mut Vec<ImportInfo>) {
        ...
        for i in 0..node.child_count() {
            if let Some(child) = node.child(i) {
                ...
                } else if matches!(child.kind(), "dotted_as_names" | "dotted_as_name") {
    ...
    fn process_import_from_statement(
        ...
        for i in 0..node.child_count() {
            if let Some(child) = node.child(i) {
                ...
                match child.kind() {
                    "import_list" => self.process_import_list(child, source, imports),
                    "aliased_import" => self.process_single_as_name(child, source, imports),
    ...
    fn process_dotted_as_names(&self, node: Node, source: &str, imports: &mut Vec<ImportInfo>) {
        if matches!(node.kind(), "dotted_as_name" | "aliased_import") {
            self.process_single_as_name(node, source, imports);
            return;
        }

        for i in 0..node.child_count() {
            if let Some(child) = node.child(i) {
                if matches!(child.kind(), "dotted_as_name" | "aliased_import") {
                    self.process_single_as_name(child, source, imports);
                } else if child.kind() == "dotted_name" || child.kind() == "identifier" {
                    if let Ok(name) = child.utf8_text(source.as_bytes()) {
                        imports.push(ImportInfo {
