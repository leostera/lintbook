...
impl Rule for LateFutureImport {
    ...
    fn check(&self, tree: &Tree, source: &str) -> Vec<LintViolation> {
        ...
        self.visit_module_statements(
            root_node,
            source,
            &mut violations,
            &mut has_non_future_statement,
            &mut first_non_future_line,
        );
        violations
    }
}
...
impl LateFutureImport {
    fn visit_module_statements(
        &self,
        node: Node,
        source: &str,
        violations: &mut Vec<LintViolation>,
        has_non_future_statement: &mut bool,
        first_non_future_line: &mut usize,
    ) {
    ...
    fn is_module_docstring(&self, node: Node, position: usize) -> bool {
        ...
        if position > 2 {
            // Allow some flexibility for comments
            return false;
        }
    ...
    fn check_nested_future_imports(
        &self,
        node: Node,
        source: &str,
        violations: &mut Vec<LintViolation>,
        ...
        if (node.kind() == "future_import_statement"
            || node.kind() == "import_from_statement" && self.is_future_import(node, source))
            && node.parent().map(|parent| parent.kind()) != Some("module")
        {
            // Future import found in nested scope - always invalid
            self.report_violation(node, 0, violations, source);
            return;
        }
    ...
    fn report_violation(
        ...
        let message = if first_non_future_line > 0 {
            format!(
                "__future__ import '{}' must occur at the beginning of the file (before line {})",
                import_text, first_non_future_line
            )
        } else {
