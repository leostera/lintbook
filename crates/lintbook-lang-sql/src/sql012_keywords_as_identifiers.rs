...
impl KeywordsAsIdentifiers {
    fn check_identifier_usage(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if lower_line.trim_start().starts_with("select ") {
                self.check_select_identifiers(line, line_idx, violations, node, &sql_keywords);
            }
        }
    }
    ...
    fn check_select_identifiers(
        ...
        let lower_line = line.to_lowercase();
        let Some(select_pos) = lower_line.find("select") else {
            return;
        };
        let from_pos = lower_line.find(" from ").unwrap_or(line.len());
        if select_pos + 6 >= from_pos {
            return;
        }

        let select_list = &line[select_pos + 6..from_pos];
        for part in select_list.split(',') {
            ...
            if !identifier.is_empty() && keywords.contains(&lower_identifier.as_str()) {
                let start_pos = node.start_position();
                violations.push(LintViolation {
                    line: start_pos.row + line_idx + 1,
                    column: start_pos.column + 1,
                    message: format!(
                        "Identifier '{}' is a SQL keyword. Consider using a different name or quoting it",
                        identifier
                    ),
                    lint_name: self.name().to_string(),
                    lint_id: self.id().to_string(),
