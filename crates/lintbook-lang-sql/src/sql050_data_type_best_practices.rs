...
impl DataTypeBestPractices {
    ...
    fn check_deprecated_types(
        ...
        let deprecated_types = [
            ...
            (
                "timestamp",
                "Use ROWVERSION for SQL Server or consider DATETIME2",
            ),
            (
                "datetime",
                "Consider DATETIME2 for better precision (SQL Server)",
            ),
            (
                "smalldatetime",
                "Consider DATETIME2 for better range and precision",
            ),
        ];
    ...
    fn check_varchar_without_length(
            ...
            || lower_line.contains(" varchar)")
        {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "VARCHAR without explicit length may default to 1. Specify length: VARCHAR(n)"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        if lower_line.contains(" nvarchar ")
            || lower_line.contains(" nvarchar,")
            || lower_line.contains(" nvarchar)")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "NVARCHAR without explicit length may default to 1. Specify length: NVARCHAR(n)"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
    ...
    fn check_float_precision(
        ...
        if lower_line.contains(" float ")
            || lower_line.contains(" float,")
            || lower_line.contains(" float)")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
        ...
        if lower_line.contains(" real ")
            || lower_line.contains(" real,")
            || lower_line.contains(" real)")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
    ...
    fn check_money_types(
        ...
        if lower_line.contains(" money ")
            || lower_line.contains(" money,")
            || lower_line.contains(" money)")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
        ...
        if lower_line.contains(" smallmoney ")
            || lower_line.contains(" smallmoney,")
            || lower_line.contains(" smallmoney)")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
