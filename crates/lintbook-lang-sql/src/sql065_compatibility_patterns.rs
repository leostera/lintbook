...
impl CompatibilityPatterns {
    ...
    fn check_vendor_specific_types(
        ...
        let sqlserver_types = [
            (
                "uniqueidentifier",
                "GUID type - not portable. Consider CHAR(36) for cross-platform compatibility",
            ),
            (
                "money",
                "SQL Server specific. Use DECIMAL(19,4) for cross-platform compatibility",
            ),
            (
                "smallmoney",
                "SQL Server specific. Use DECIMAL(10,4) for cross-platform compatibility",
            ),
            (
                "datetime2",
                "SQL Server specific. Use TIMESTAMP or DATETIME for better portability",
            ),
            (
                "datetimeoffset",
                "SQL Server specific. Consider separate date and timezone columns",
            ),
            ("hierarchyid", "SQL Server specific hierarchical data type"),
            (
                "geography",
                "SQL Server spatial type - check if target databases support spatial data",
            ),
            (
                "geometry",
                "SQL Server spatial type - check if target databases support spatial data",
        ...
        let mysql_types = [
            (
                "tinyint",
                "MySQL specific. Use SMALLINT for better portability",
            ),
            (
                "mediumint",
                "MySQL specific. Use INT for better portability",
            ),
            (
                "enum(",
                "MySQL specific. Consider lookup table for cross-platform compatibility",
            ),
    ...
    fn check_vendor_specific_functions(
        ...
        let sqlserver_functions = [
            (
                "newid()",
                "SQL Server specific. Use UUID() in MySQL or random values in other databases",
            ),
            (
                "getdate()",
                "SQL Server specific. Use NOW() (MySQL) or CURRENT_TIMESTAMP (standard)",
            ),
            (
                "getutcdate()",
                "SQL Server specific. Use UTC_TIMESTAMP() in MySQL",
            ),
            ("sysdatetime()", "SQL Server specific. Limited portability"),
            (
                "datediff(",
                "Function signature varies between databases. Check parameter order",
            ),
            (
                "dateadd(",
                "SQL Server/Access specific. Use INTERVAL in other databases",
            ),
            (
                "charindex(",
                "SQL Server specific. Use INSTR() or POSITION() for portability",
            ),
            (
                "len(",
                "SQL Server specific. Use LENGTH() or CHAR_LENGTH() for portability",
            ),
            (
                "stuff(",
                "SQL Server specific. Use REPLACE() or string concatenation",
            ),
        ...
        let mysql_functions = [
            (
                "concat_ws(",
                "MySQL specific. Concatenate manually for cross-platform compatibility",
            ),
            (
                "group_concat(",
                "MySQL specific. Use STRING_AGG() in SQL Server or manual aggregation",
            ),
            (
                "ifnull(",
                "MySQL specific. Use ISNULL() (SQL Server) or COALESCE() (standard)",
            ),
    ...
    fn check_sql_standard_compliance(
        ...
        if lower_line.contains(" over ")
            && !lower_line.contains("partition by")
        ...
        if lower_line.contains("isnull(") {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "ISNULL() function name varies. COALESCE() is SQL standard and more portable"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
