...
impl DeprecatedFeatures {
    ...
    fn check_deprecated_data_types(
        ...
        let deprecated_types = [
            (
                "text",
                "Use VARCHAR(MAX) instead of TEXT for large text data",
            ),
            (
                "ntext",
                "Use NVARCHAR(MAX) instead of NTEXT for large Unicode text",
            ),
            (
                "image",
            ...
            (
                "timestamp",
                "Use ROWVERSION instead of TIMESTAMP for versioning",
            ),
            (
                "sql_variant",
                "Consider specific data types instead of SQL_VARIANT when possible",
            ),
        ];
    ...
    fn check_deprecated_functions(
        ...
        let deprecated_functions = [
            (
                "datalength(",
                "Consider LEN() for string length or use specific type functions",
            ),
            (
                "getdate()",
                "Consider SYSDATETIME() for higher precision or GETUTCDATE() for UTC",
            ),
            ("host_id()", "Function deprecated - no direct replacement"),
            (
                "is_member(",
                "Use IS_ROLEMEMBER() for role membership checks",
            ),
        ...
        for (deprecated_func, suggestion) in deprecated_functions.iter() {
            if lower_line.contains(deprecated_func) {
                ...
                violations.push(LintViolation {
                    ...
                    message: format!(
                        "Deprecated function '{}'. {}",
                        deprecated_func.trim_end_matches('(').to_uppercase(),
                        suggestion
                    ),
                    lint_name: self.name().to_string(),
                    lint_id: self.id().to_string(),
    ...
    fn check_deprecated_syntax(
        ...
        if lower_line.contains("set rowcount") && !lower_line.contains("set rowcount 0") {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "SET ROWCOUNT is deprecated for DML statements. Use TOP clause instead"
                    .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        let deprecated_dbcc = [
            ...
            (
                ...
                "Use DBCC SHOW_STATISTICS with new syntax",
            ),
        ];
    ...
    fn check_deprecated_system_objects(
        ...
        let deprecated_objects = [
            ...
            (
                "sysprocesses",
                "Use sys.dm_exec_sessions and sys.dm_exec_requests",
            ),
            ("syslocks", "Use sys.dm_tran_locks"),
            (
                "information_schema.columns",
                "Consider sys.columns for better performance",
            ),
        ];
        ...
        if lower_line.contains("sys.sql_dependencies") {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "sys.sql_dependencies is deprecated. Use sys.sql_expression_dependencies"
                    .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
    ...
    fn check_deprecated_join_syntax(
            ...
            && lower_line.contains(" where ")
        {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "Old-style comma join syntax. Use explicit INNER JOIN for better readability"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
