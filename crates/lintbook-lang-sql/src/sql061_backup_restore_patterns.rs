...
impl BackupRestorePatterns {
    fn check_backup_restore_patterns(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if lower_line.contains("restore database")
                || lower_line.contains("restore filelistonly")
            {
    ...
    fn check_backup_operations(
        ...
        if lower_line.contains("to disk") {
            ...
            if !lower_line.contains("2023")
                && !lower_line.contains("2024")
                && !lower_line.contains("202")
            {
                let start_pos = node.start_position();
                violations.push(LintViolation {
                    line: start_pos.row + line_idx + 1,
    ...
    fn check_restore_operations(
        ...
        if lower_line.contains("restore database")
            && !lower_line.contains("move")
            && !lower_line.contains("replace")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message:
                    "Database restore without MOVE or REPLACE. Ensure target location is correct"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        if lower_line.contains("restore database") && !lower_line.contains("stats") {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "Database restore without STATS option. Consider STATS for progress monitoring"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        if lower_line.contains("dbcc checkdb") {
            if !lower_line.contains("with no_infomsgs") {
                ...
                violations.push(LintViolation {
                    ...
                    column: start_pos.column + 1,
                    message:
                        "DBCC CHECKDB without NO_INFOMSGS. Consider adding to reduce output volume"
                            .to_string(),
                    lint_name: self.name().to_string(),
                    lint_id: self.id().to_string(),
                });
    ...
    fn check_log_backup_patterns(
        ...
        if lower_line.contains("to disk")
            && !lower_line.contains(".trn")
            && !lower_line.contains(".log")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
