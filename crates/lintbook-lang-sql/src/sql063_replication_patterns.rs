...
impl ReplicationPatterns {
    ...
    fn check_non_replicable_operations(
        ...
        if lower_line.contains("#")
            && (lower_line.contains("create table") || lower_line.contains("insert into"))
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
    ...
    fn check_identity_replication_issues(
        ...
        if lower_line.contains("set identity_insert") {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "SET IDENTITY_INSERT operation. May cause conflicts in replication scenarios"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        if lower_line.contains("uniqueidentifier")
            && lower_line.contains("default")
            && lower_line.contains("newid")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message:
                    "UNIQUEIDENTIFIER with NEWID() default. Good choice for replication scenarios"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
