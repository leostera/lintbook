...
impl LineBreaksInClauses {
    fn check_clause_line_breaks(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if found_clauses.len() > 1 {
                ...
                let clause_names: Vec<&str> =
                    found_clauses.iter().map(|(_, name)| **name).collect();
    ...
    fn check_specific_clause_patterns(
                ...
                if let Some(from_pos) = lower_line.find(" from ") {
                    ...
                    if content_between.len() > 30 || content_between.contains(',') {
                        ...
                        violations.push(LintViolation {
                            ...
                            message: "FROM clause should be on a new line for better readability"
                                .to_string(),
                            lint_name: self.name().to_string(),
                            lint_id: self.id().to_string(),
                        });
        ...
        if lower_line.contains(" where ") {
            let and_or_count =
                lower_line.matches(" and ").count() + lower_line.matches(" or ").count();
            if and_or_count >= 2 {
                let start_pos = node.start_position();
                violations.push(LintViolation {
                    line: start_pos.row + line_idx + 1,
                    column: start_pos.column + 1,
                    message: "Complex WHERE conditions should be split across multiple lines"
                        .to_string(),
                    lint_name: self.name().to_string(),
                    lint_id: self.id().to_string(),
                });
