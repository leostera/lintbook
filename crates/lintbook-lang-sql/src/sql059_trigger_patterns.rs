...
impl TriggerPatterns {
    fn check_trigger_patterns(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if lower_line.contains("after insert")
                || lower_line.contains("before insert")
                || lower_line.contains("after update")
                || lower_line.contains("before update")
                || lower_line.contains("after delete")
                || lower_line.contains("before delete")
            {
    ...
    fn check_trigger_definition(
        ...
        if lower_line.contains("disable trigger") {
            ...
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
                column: start_pos.column + 1,
                message: "Trigger being disabled. Ensure this is temporary and document the reason"
                    .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        if lower_line.contains("create trigger") {
            ...
            if let Some(trigger_start) = lower_line.find("create trigger") {
                ...
                if let Some(space_pos) = after_trigger.find(' ') {
                    ...
                    if !trigger_name.contains("audit")
                        && !trigger_name.contains("log")
                        && !trigger_name.contains("validate")
                        && !trigger_name.contains("update")
                        && !trigger_name.contains("insert")
                        && !trigger_name.contains("delete")
                    {
                        let start_pos = node.start_position();
                        violations.push(LintViolation {
                            line: start_pos.row + line_idx + 1,
    ...
    fn check_trigger_events(
        ...
        let event_count = ["insert", "update", "delete"]
            .iter()
        ...
        if lower_line.contains("after delete") {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "AFTER DELETE trigger. Ensure this doesn't conflict with foreign key cascades"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
    ...
    fn check_recursion_risks(
        ...
        if lower_line.contains("trigger_nestlevel") {
            ...
            violations.push(LintViolation {
                ...
                column: start_pos.column + 1,
                message:
                    "TRIGGER_NESTLEVEL() detected. Good practice for preventing infinite recursion"
                        .to_string(),
                lint_name: self.name().to_string(),
                lint_id: self.id().to_string(),
            });
        ...
        if (lower_line.contains("inserted") || lower_line.contains("deleted"))
            && !lower_line.contains("exists")
        {
            let start_pos = node.start_position();
            violations.push(LintViolation {
                line: start_pos.row + line_idx + 1,
