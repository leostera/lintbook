...
impl ExplicitUnionAll {
    fn check_union_statements(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            while let Some(relative_pos) = lower_line[search_start..].find("union") {
                let union_pos = search_start + relative_pos;
                let after_keyword = &lower_line[union_pos + 5..];
                // Get text after UNION keyword
                let after_union = after_keyword.trim_start();
                ...
                if !after_union.starts_with("all") && !after_union.starts_with("distinct") {
                    ...
                    let is_standalone = (before_union.is_empty()
                        ...
                        && (after_union.is_empty()
                            || after_keyword.starts_with(|c: char| !c.is_alphanumeric()));
