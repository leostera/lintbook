...
impl PartitionPatterns {
    fn check_partition_patterns(
        ...
        for (line_idx, line) in lines.iter().enumerate() {
            ...
            if lower_line.contains("on ")
                && (lower_line.contains("create table") || lower_line.contains("create index"))
            {
    ...
    fn check_partition_candidates(
        ...
        if lower_line.contains("where")
            && (lower_line.contains("date")
                || lower_line.contains("created")
                || lower_line.contains("id"))
        {
